...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<'daily' | 'weekly' | 'monthly' | 'history' | 'saved' | 'single'>('daily');
  ...
  // Local state for profile editing with selectors
  const [editingForbidden, setEditingForbidden] = useState('');
  const [editingPreferred, setEditingPreferred] = useState('');
  const [editingAvailable, setEditingAvailable] = useState('');
  const [selectedIngredientsForSingleMeal, setSelectedIngredientsForSingleMeal] = useState('');
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      ...
      if (!parsed.cookingLevel) parsed.cookingLevel = 'intermediate';
      setProfile(parsed);
  ...
  const handleEditProfile = () => {
    setEditingForbidden(profile?.forbiddenIngredients || translations[lang].defaults.forbidden);
    setEditingPreferred(profile?.preferredIngredients || translations[lang].defaults.preferred);
    setEditingAvailable(profile?.availableIngredients || translations[lang].defaults.available);
  ...
  const handleSaveProfile = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newProfile: UserProfile = {
      ...
      healthGoals: (formData.get('healthGoals') as string).split(',').map(s => s.trim()).filter(Boolean),
      dietType: formData.get('dietType') as string,
      countries: (formData.get('countries') as string).split(',').map(s => s.trim()).filter(Boolean),
      dietaryRestrictions: (formData.get('dietaryRestrictions') as string).split(',').map(s => s.trim()).filter(Boolean),
      allergies: (formData.get('allergies') as string).split(',').map(s => s.trim()).filter(Boolean),
      forbiddenIngredients: editingForbidden,
      preferredIngredients: editingPreferred,
      availableIngredients: editingAvailable,
      ...
      cookingLevel: formData.get('cookingLevel') as any,
  ...
  const handleExcludeIngredient = async (ingredient: string, mealType?: 'breakfast' | 'lunch' | 'snack' | 'dinner', dayIndex?: number, weekIndex?: number) => {
    ...
    if (currentForbidden.includes(cleanIngredient.toLowerCase())) {
      ...
      // Check if it's a fish or seafood
      const fishKeywords = [
        'salmón', 'salmon', 'lubina', 'atún', 'atun', 'bacalao', 'merluza', 'dorada', 'trucha', 'sardina', 'boquerón', 'boqueron', 
        'pescado', 'fish', 'tuna', 'cod', 'hake', 'sea bass', 'trout', 'sardine', 'anchovy', 'lenguado', 'rape', 'rodaballo', 
        'caballa', 'bonito', 'emperador', 'pez espada', 'marisco', 'shellfish', 'prawn', 'shrimp', 'gambas', 'langostino', 
        'mejillón', 'mejillon', 'almeja', 'pulpo', 'octopus', 'squid', 'calamar', 'sepia', 'bogavante', 'crab', 'cangrejo'
      ];
      const isFish = fishKeywords.some(k => cleanIngredient.toLowerCase().includes(k));

      let newForbidden = profile.forbiddenIngredients 
        ? `${profile.forbiddenIngredients}, ${cleanIngredient}`
        : cleanIngredient;
      
      // If it's a fish and "pescado" is not yet in the list, add it too
      if (isFish && !currentForbidden.includes('pescado')) {
        newForbidden += ', pescado';
      }
  ...
  const handleGenerateSingleMeal = async (mealType: 'breakfast' | 'lunch' | 'snack' | 'dinner') => {
    ...
    try {
      ...
      const meal = await generateSingleMeal(profile, mealType, lang, selectedIngredientsForSingleMeal);
  ...
  const generateFullHTML = (planData: any, mode: string) => {
    ...
    if (planData) {
      if (mode === 'monthly') {
        contentHTML = planData.weeks.map((week: any, wIdx: number) => `
          <div class="space-y-6 mb-12">
            ...
            ${week.days.map((day: any) => generateDayHTML(day)).join('')}
          </div>
        `).join('');
      } else if (mode === 'weekly') {
        contentHTML = generateShoppingListHTML(planData.shoppingList) + planData.days.map((day: any) => generateDayHTML(day)).join('');
      } else if (mode === 'daily') {
        contentHTML = generateDayHTML(planData);
      } else if (mode === 'single') {
        contentHTML = `
          <div class="max-w-2xl mx-auto">
            ${generateMealHTML(planData.meal, t.meals[planData.type as keyof typeof t.meals])}
          </div>
        `;
      }
    } else {
...
<body class="p-4 md:p-8">
  ...
  const handleDownloadHTML = () => {
    ...
    if (!planData) return;
    const htmlContent = generateFullHTML(planData, mode);
    ...
    const dateStr = new Date().toISOString().split('T')[0];
    const titleStr = planData && 'title' in (planData as any) 
      ? (planData as any).title.replace(/[^a-z0-9]/gi, '_').toLowerCase() 
      : (mode === 'single' ? (planData as any).meal.name.replace(/[^a-z0-9]/gi, '_').toLowerCase() : 'Plan');
  ...
  const handleShareWhatsApp = async () => {
    ...
    } else if (mode === 'single' && singleMeal) {
      const { meal, type } = singleMeal;
      textSummary += `🍳 *${t.meals[type as keyof typeof t.meals]}:* ${meal.name}\n`;
      textSummary += `\n_${meal.description}_`;
      textSummary += `\n\n*Ingredientes:* \n${meal.ingredients.join(', ')}`;
      if (meal.proTip) textSummary += `\n\n💡 *Tip:* ${meal.proTip}`;
    ...
    } else if (mode === 'monthly' && monthlyPlan) {
      ...
      <main className="max-w-5xl mx-auto px-6 mt-12 space-y-12">
            ...
            {isEditingProfile ? (
              ...
              >
                <form onSubmit={handleSaveProfile} className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="md:col-span-4 border-t border-stone-100 pt-6">
                    <IngredientSelector 
                      label={t.forbidden} 
                      icon={<AlertCircle size={14} className="text-red-500" />}
                      value={editingForbidden}
                      onChange={setEditingForbidden}
                      t={t}
                      lang={lang}
                    />
                  </div>
                  ...
                  <div className="md:col-span-4 border-t border-stone-100 pt-6">
                    <IngredientSelector 
                      label={t.preferred} 
                      icon={<CheckCircle2 size={14} className="text-med-olive" />}
                      value={editingPreferred}
                      onChange={setEditingPreferred}
                      t={t}
                      lang={lang}
                    />
                  </div>
                  ...
                  <div className="md:col-span-4 border-t border-stone-100 pt-6">
                    <IngredientSelector 
                      label={t.available} 
                      icon={<Utensils size={14} className="text-emerald-600" />}
                      value={editingAvailable}
                      onChange={setEditingAvailable}
                      t={t}
                      lang={lang}
                    />
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-xs font-bold uppercase text-stone-700 tracking-wider">{t.cookingLevel}</label>
                    <select name="cookingLevel" defaultValue={profile?.cookingLevel || 'intermediate'} className="input-med">
                      {Object.entries(t.cookingLevels).map(([key, label]) => (
                        <option key={key} value={key}>{label}</option>
                      ))}
                    </select>
                  </div>
        ...
        {!isEditingProfile && (
          <section className="flex flex-col items-center gap-6">
            ...
            <div className="flex flex-col items-center gap-6 mt-8 p-8 bg-stone-50/50 rounded-3xl border border-stone-100 max-w-2xl mx-auto w-full">
              <div className="text-center space-y-2">
                <h3 className="text-xl font-serif text-stone-800">{t.generateSingle}</h3>
                <p className="text-xs text-stone-500 uppercase tracking-widest">{lang === 'es' ? 'Selecciona ingredientes para tu receta personalizada' : 'Select ingredients for your custom recipe'}</p>
              </div>

              <div className="w-full">
                <IngredientSelector 
                  label={t.ingredients}
                  icon={<Utensils size={14} className="text-med-olive" />}
                  value={selectedIngredientsForSingleMeal}
                  onChange={setSelectedIngredientsForSingleMeal}
                  t={t}
                  lang={lang}
                />
              </div>
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="flex flex-wrap justify-center gap-2">
                  {(['breakfast', 'lunch', 'snack', 'dinner'] as const).map((type) => (
                    <button
                      ...
                      disabled={isLoading}
                      className="btn-med bg-med-olive text-white hover:bg-med-olive/90 text-sm px-6 py-2.5 rounded-full transition-all shadow-lg shadow-med-olive/20 flex items-center gap-2"
                    >
                      <Zap size={14} />
                      {t.meals[type]}
                    </button>
                ...
                {selectedIngredientsForSingleMeal && (
                  <button 
                    onClick={() => setSelectedIngredientsForSingleMeal('')}
                    className="text-[10px] font-bold uppercase text-stone-400 hover:text-red-500 transition-colors"
                  >
                    {lang === 'es' ? 'Limpiar ingredientes seleccionados' : 'Clear selected ingredients'}
                  </button>
                )}
              </div>
            </div>
        ...
        <AnimatePresence mode="wait">
          {singleMeal && viewMode === 'daily' && (
            ...
            >
              ...
              <TokenCalculator usage={singleMeal.meal.usage} t={t} />
            </motion.section>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          ...
          {currentPlan && viewMode !== 'history' && (
            ...
            >
              ...
              <TokenCalculator usage={currentPlan.usage} t={t} />
            </motion.section>
          )}
          ...
          {weeklyPlan && (
            ...
            >
              ...
              <TokenCalculator usage={weeklyPlan.usage} t={t} />
            </motion.section>
          )}
          ...
          {monthlyPlan && (
            ...
            >
              ...
              <TokenCalculator usage={monthlyPlan.usage} t={t} />
            </motion.section>
          )}
        </AnimatePresence>
      </main>
...
function IngredientSelector({ 
  ...
  const toggleIngredient = (ingredient: string) => {
    const currentList = value.split(',').map(i => i.trim()).filter(i => i !== "");
    const lowerIng = ingredient.toLowerCase();
    
    if (currentList.some(i => i.toLowerCase() === lowerIng)) {
      onChange(currentList.filter(i => i.toLowerCase() !== lowerIng).join(', '));
    } else {
      onChange([...currentList, ingredient].join(', '));
    }
  };

  const isSelected = (ingredient: string) => {
    return value.split(',').map(i => i.trim().toLowerCase()).includes(ingredient.toLowerCase());
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {FOOD_DATABASE.map((cat) => (
          <button
            key={cat.categoria}
            type="button"
            onClick={() => setActiveCategory(activeCategory === cat.categoria ? null : cat.categoria)}
            className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-tight transition-all border ${
              activeCategory === cat.categoria 
                ...
                : 'bg-stone-50 text-stone-600 border-stone-200 hover:border-med-olive/30'
            }`}
          >
            {cat.categoria}
          </button>
      ...
      <AnimatePresence>
        {activeCategory && (
          <motion.div
            ...
            className="overflow-hidden bg-stone-50 rounded-xl border border-stone-100 p-4"
          >
            <div className="space-y-4">
              {FOOD_DATABASE.find(c => c.categoria === activeCategory)?.items?.map(item => (
                <button
                  key={item}
                  type="button"
                  onClick={() => toggleIngredient(item)}
                  className={`mr-2 mb-2 px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                    isSelected(item)
                      ? 'bg-med-olive text-white shadow-sm'
                      : 'bg-white text-stone-600 border border-stone-200 hover:border-med-olive/50'
                  }`}
                >
                  {item}
                </button>
              ))}
              {FOOD_DATABASE.find(c => c.categoria === activeCategory)?.subcategorias?.map(sub => (
                <div key={sub.nombre} className="space-y-2">
                  <h5 className="text-[9px] font-bold uppercase text-stone-400 tracking-widest">{sub.nombre}</h5>
      ...
      <textarea 
        ...
        className="input-med h-24 placeholder:italic placeholder:text-stone-300" 
        placeholder={lang === 'es' ? "Selecciona arriba o escribe aquí separados por comas..." : "Select above or type here separated by commas..."}
      />
    </div>
  );
}

function TokenCalculator({ usage, t }: { usage?: { promptTokens: number, candidatesTokens: number, totalTokens: number }, t: any }) {
  if (!usage) return null;

  // Gemini 1.5 Flash Pricing (approximate)
  // Input: $0.075 / 1M tokens
  // Output: $0.30 / 1M tokens
  const inputCost = (usage.promptTokens * 0.075) / 1000000;
  const outputCost = (usage.candidatesTokens * 0.30) / 1000000;
  const totalCost = inputCost + outputCost;

  return (
    <div className="card-med p-6 bg-stone-50 border border-stone-200 mt-8">
      <div className="flex items-center gap-2 mb-4 text-med-olive">
        <FileCode size={20} />
        <h3 className="text-xl font-serif">{t.tokenUsage}</h3>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div>
          <p className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-1">{t.inputTokens}</p>
          <p className="text-lg font-bold text-stone-700">{usage.promptTokens.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-1">{t.outputTokens}</p>
          <p className="text-lg font-bold text-stone-700">{usage.candidatesTokens.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-1">{t.totalTokens}</p>
          <p className="text-lg font-bold text-med-olive">{usage.totalTokens.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-1">{t.estimatedCost}</p>
          <p className="text-lg font-bold text-emerald-600">${totalCost.toFixed(5)}</p>
        </div>
      </div>
...
function MealCard({ type, meal, t, onExclude, onRegenerate, onLike, onDislike, isLoading }: { type: string, meal: Meal, t: any, onExclude?: (ing: string) => void, onRegenerate?: () => void, onLike?: () => void, onDislike?: () => void, isLoading?: boolean }) {
  ...
  const handleDownloadAsPhoto = async () => {
    const cardElement = document.getElementById(`meal-card-${meal.name.replace(/\s+/g, '-').toLowerCase()}`);
    if (!cardElement) return;

    try {
      // Find the footer/buttons section and temporarily hide it
      const buttonsDiv = cardElement.querySelector('.meal-card-buttons');
      ...
      if (buttonsDiv) (buttonsDiv as HTMLElement).style.display = 'flex';

      const link = document.createElement('a');
      link.download = `${meal.name.replace(/\s+/g, '_').toLowerCase()}.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
    } catch (error) {
      console.error('Error rendering meal to image:', error);
    }
  };

  return (
    <div 
      id={`meal-card-${meal.name.replace(/\s+/g, '-').toLowerCase()}`}
      className="card-med flex flex-col group/card relative"
    >
      {isLoading && (
      ...
      <div className="p-6 flex-1">
        ...
        <div className="flex gap-2 mb-4 meal-card-buttons">
          <button 
          ...
          <button 
            onClick={handleDownloadAsPhoto}
            className="btn-med border border-stone-200 text-stone-400 hover:text-med-olive hover:bg-stone-50 px-3 py-2"
